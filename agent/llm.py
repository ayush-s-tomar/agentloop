...
# ---------- model aliases ----------
FAST_MODEL   = os.environ.get("GROQ_FAST_MODEL", "openai/gpt-oss-20b")   # high TPM limit on free tier ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use for research loops
REASON_MODEL = os.environ.get("GROQ_REASON_MODEL", "openai/gpt-oss-120b")  # smarter ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use for plan / reflect / synthesise
...
# and set GROQ_REASON_MODEL in your .env ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â do not hardcode it here.
...
def _call(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            log.warning(
                "Rate-limited (attempt %d/%d). Waiting %.1fs ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦",
            ...
            if exc.status_code in (500, 502, 503, 504) and attempt < MAX_RETRIES:
                log.warning("Groq %d error, retrying in %.1fs ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦", exc.status_code, delay)
                time.sleep(delay)
                delay = min(delay * 2, MAX_DELAY_SEC)
            else:
                raise
...
def complete(system: str, user: str, json_mode: bool = False) -> str:
    ...
    Set json_mode=True when you expect a JSON response ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â strips fences automatically.
    """
    raw = _call(
        messages=[
...
def plan(topic: str, prior_notes: list[str]) -> list[str]:
    """Return 3ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“4 sub-questions to research."""
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "No markdown, no preamble ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â raw JSON array only."
                ),
            },
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "rankings, availability, etc.) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â when in doubt, say YES.\n"
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "write a concise 2-4 sentence answer. Cite no sources by URL ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â just facts."
                ),
            },
            {
                "role": "user",
